...
█▀█ █▀▄ ██▄ ▄█   █▄▄ █▄█ █░▀░█ █▄█ █▀█ ░█░   █░▀░█ █▄█ █▄▀ █▄█ █▄▄ ██▄", ConsoleColor.Red);
            ...
            Utils.LbL($"\n{chosen} targeted.", ConsoleColor.Red, 40);
            BodyPartData targetPart = TargetBodyPart(weaponSkill, chosen);
            Utils.LbL($"\n{targetPart.name} locked in.", ConsoleColor.Red, 40);
        ...
        {
            Utils.LbL($"\n## Equipped Weapons ##", ConsoleColor.Cyan, 20);
        ...
        private static HumanEntity TargetEnemy(List<HumanEntity> npcs)
        {
            ...
            {
                Utils.LbL($"\n{i + 1}. {npcs[i]}", ConsoleColor.Yellow, 20);
            }
            ...
            if (attemptAttack <= bodyPart.baseHitChance + skill.level)
            {
                ...
                switch (item.typeSkill)
                {
                    case Item.SkillType.BladeMeleeCombat:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name.ToLower()} slashed with {item.name}.", ConsoleColor.Red, 20);
                        }
                        break;
                    case Item.SkillType.BluntMeleeCombat:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name.ToLower()} bludgeoned with {item.name}.", ConsoleColor.Red, 20);
                        }
                        break;
                    case Item.SkillType.UnarmedMeleeCombat:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name.ToLower()} struck with {item.name}.", ConsoleColor.Red, 20);
                        }
                        break;
                    case Item.SkillType.OneHandFirearms:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name.ToLower()} hit with {item.name}.", ConsoleColor.Red, 20);
                        }
                        break;
        ...
        private static void EnemyAttemptAttack(HumanEntity npc, Skill skill, Item item, HumanEntity player, BodyPartData bodyPart)
        {
            ...
            if (attemptAttack <= bodyPart.baseHitChance + skill.level)
            {
                ...
                switch (item.typeSkill)
                {
                    case Item.SkillType.BladeMeleeCombat:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name} slashed with {item.name}.", ConsoleColor.Green, 20);
                        }
                        break;
                    case Item.SkillType.BluntMeleeCombat:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name} bludgeoned with {item.name}.", ConsoleColor.Green, 20);
                        }
                        break;
                    case Item.SkillType.UnarmedMeleeCombat:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name} struck with {item.name}.", ConsoleColor.Green, 20);
                        }
                        break;
                    case Item.SkillType.OneHandFirearms:
                        {
                            Utils.LbL($"\n{npc.name}'s {bodyPart.name} hit with {item.name}.", ConsoleColor.Green, 20);
                        }
                        break;
                    case Item.SkillType.TwoHandFirearms:
                        {
            ...
            {
                Utils.LbL("\nAttack missed.", ConsoleColor.DarkGray, 20);
            }
        }
    }
}
